...
void LCD_Init(void)
{
	...
	LCD_CMD_Buffer |= SPI_SCK+SPI_CS;  //SS和SCK预先设置为高电平
	
	LCD_CMD_Buffer &= ~(SPI_RES);       //复位LCD模块
	TimeDelay(2);			
	LCD_CMD_Buffer |= SPI_RES;		    //保持低电平大概2ms左右
	LCD_CMD_Dir &= ~SPI_RES;            //释放LCD的复位端口
	TimeDelay(8);				    	//延时大概10ms左右
}
//========================================================================
